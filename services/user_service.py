...
class UserService:
    ...
    def register_user(request, db):
        logger.debug(
            f"Received registration request: username={request.username}, role={request.role}"
        )
        if len(request.password.encode("utf-8")) > 72:
            logger.info(
                f"Registration failed: Password too long (bytes) for username '{request.username}'."
            )
            return {
                "error": "Password cannot be longer than 72 bytes (ASCII or UTF-8)."
            }
        try:
            ...
            user = User(
                username=request.username, hashed_password=hashed_pw, role=request.role
            )
            db.add(user)
            db.commit()
            db.refresh(user)
